...
In scientific research, a study is considered to be well executed if it has 4 characteristics: reproducible, replicable, robust, and generalizable. For the purpose of this assignment, I will focus on the first two.  
Reproducibility means that the same data can be analysed in the same way multiple times, with the same results/conclusions being reached every time. This requires for all the data and analysis methods to be provided in a self-explanatory form, so that e.g., a third person can recreate the whole process. In contrast, replicability involves the use of DIFFERENT data, which is being analysed in the same way, yielding the same results. This requires specifically the methods to be applicable to different datasets, and to be open-access, so that researchers that have collected their own data can apply them. For replicability, platforms such as protocols.io are very useful, since they outline a standardized way (protocol) for data to be collected, which allows analyses to be more easily applied to different datasets.  
...
* GitHub is cloud-based. This is important, because if a researcher’s own device was to stop working, the data and analysis would not be lost.  
...
References:  
The turing way. Definitions - The Turing Way. https://the-turing-way.netlify.app/reproducible-research/overview/overview-definitions.html  
